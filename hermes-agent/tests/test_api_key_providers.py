"""Tests for API-key provider support (z.ai/GLM, Kimi, MiniMax, EvoLink)."""
...
PROVIDER_ENV_VARS = (
    ...
    "EVOLINK_API_KEY", "EVOLINK_BASE_URL", "OPENAI_BASE_URL",
)
...
class TestResolveApiKeyProviderCredentials:
    ...
    def test_resolve_evolink_with_key(self, monkeypatch):
        monkeypatch.setenv("EVOLINK_API_KEY", "ev-secret-key")
        creds = resolve_api_key_provider_credentials("evolink")
        assert creds["provider"] == "evolink"
        assert creds["api_key"] == "ev-secret-key"
